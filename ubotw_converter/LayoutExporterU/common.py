...
class BlockHeader:
    def __init__(self, file, pos, endian='>'):
        self.magic, self.size = struct.unpack_from(f'{endian}4sI', file, pos)
...
class Section:
    def __init__(self, file, pos, endian='>'):
        self.blockHeader = BlockHeader(file, pos, endian)
        self.data = file[pos + 8:pos + self.blockHeader.size]

    def save(self, endian='>'):
        self.blockHeader.size = 8 + len(self.data)
        return b''.join([self.blockHeader.save(endian), self.data])
